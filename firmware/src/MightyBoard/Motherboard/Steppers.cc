...
bool getNextMove() {
	...
	if (current_block->decelerate_after < current_block->step_event_count) {
		if (feedrate_being_setup == 0) // we didn't have an acceleration or plateau phase
			feedrate = current_block->nominal_rate; // with no plateau, the nominal rate may never be reached
...
bool doInterrupt() {
	...
	if (is_running) {
		...
		else // prepareFeedrateIntervals() sets the feedrate_total_time to 0, making the next block a waste of time
		if (feedrate_changerate != 0) {
			// Change our feedrate. Here it's important to note that we can over/undershoot
			int32_t feedrate_diff = 0;

			// Voodoo lives here
			MultiU24X24toH16(feedrate_diff, feedrate_total_time, abs(feedrate_changerate));

			if (feedrate_changerate > 0) {
				// We offset up from the initial_rate
				feedrate = feedrate_diff + feedrate_start;
				...
				feedrate = feedrate_start - feedrate_diff;
